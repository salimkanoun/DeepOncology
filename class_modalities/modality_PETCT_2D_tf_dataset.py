...
class DataGenerator(object):
    ...
    def __init__(self,
                 ...
                 shuffle=True):
        """
        :param images_paths:         list of tuple :
                                            [{'pet_img': pet_path, 'ct_img': ct_path, 'mask_img': mask_path}, ...]
        :param batch_size:           int
        :param shuffle:              bool
        :param augmentation:         bool
        :param target_shape:         tuple, shape of generated PET, CT or MASK scan: (z, y, x) (368, 128, 128) for ex.
        :param target_voxel_spacing: tuple, resolution of the generated PET, CT or MASK scan :
                                            (z, y, x) (4.8, 4.8, 4.8) for ex.

        """
        self.images_paths = images_paths
        self.number_channels = number_channels  # PET + CT scan
    ...
    def _generator(self):
        ...
        for img_path in self.images_paths:
            ...
            # resample, align img and calculate ground truth mask from ROI
            ...
            for i in range(0, pet_array.shape[0] + 1 - n_slice):
                ...
                # pet_ct_slices = np.transpose(pet_ct_slices, (1, 2, 0))  # (z, y, x) to (x, y, z) with z as channels

                # select slices
                pet_slices = pet_array[i:i + n_slice]
                pet_slices = np.transpose(pet_slices, (1, 2, 0))  # (z, y, x) to (x, y, z) with z as channels

                ct_slices = ct_array[i:i + n_slice]
                ct_slices = np.transpose(ct_slices, (1, 2, 0))  # (z, y, x) to (x, y, z) with z as channels

                mask_array_slice = mask_array[i + n_slice // 2]

                # add one channel => (x, y, 1)
                mask_array_slice = np.expand_dims(mask_array_slice, axis=-1)

                yield pet_slices, ct_slices, mask_array_slice

    def augment(self, pet, ct, mask):
        mask = tf.cast(mask, dtype=tf.float32)
        images = np.concatenate([pet, ct, mask], axis=-1)

        if tf.random.uniform(()) > 0.5:
            rotation_range = 12.0  # in degree
            images = tf.keras.preprocessing.image.random_rotation(
                images, rotation_range, row_axis=0, col_axis=1, channel_axis=2, fill_mode='nearest'
            )

        if tf.random.uniform(()) > 0.5:
            wrg, hrg = 0.1, 0.1
            images = tf.keras.preprocessing.image.random_shift(
                images, wrg, hrg, row_axis=0, col_axis=1, channel_axis=2, fill_mode='nearest'
            )

        if tf.random.uniform(()) > 0.5:
            zoom_range = (1.1, 1.1)
            images = tf.keras.preprocessing.image.random_zoom(
                images, zoom_range, row_axis=0, col_axis=1, channel_axis=2, fill_mode='nearest'
            )

        n_slice = self.number_channels // 2
        pet = images[:, :, :n_slice]
        ct = images[:, :, n_slice:2 * n_slice]
        mask = images[:, :, 2 * n_slice:]

        return pet, ct, mask

    def normalize(self, pet, ct, mask):

        pet = pet / 25.0
        ct = (ct + 1000.0) / 2000.0

        pet = tf.clip_by_value(pet, 0.0, 1.0)
        ct = tf.clip_by_value(ct, 0.0, 1.0)

        return pet, ct, mask

    def concat(self, pet, ct, mask):
        return tf.keras.layers.concatenate([pet, ct], axis=-1), mask
    ...
    def get_dataset(self):
        ...
        if self.augmentation:
            ...
            return (dataset
                    .map(self.augment, num_parallel_calls=tf.data.experimental.AUTOTUNE)
                    .map(self.normalize, num_parallel_calls=tf.data.experimental.AUTOTUNE)  # normalize + clip
                    .map(self.concat, num_parallel_calls=tf.data.experimental.AUTOTUNE)  # stack pet & ct
