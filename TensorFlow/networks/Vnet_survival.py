...
class VnetSurvival_mixdata(object):
    ...
    def build_network(self, input_):
        ...
        #if time_horizon<150:
        #x = tf.keras.layers.Dense(256, activation=self.activation_fn, name='dense_b')(x)
        #x = tf.keras.layers.Dropout(0.2)(x)
        #if time_horizon<80:
        #x = tf.keras.layers.Dense(128, activation=self.activation_fn,name='dense_c')(x)
    ...
    def create_model(self):
        ...
        input_ = tf.keras.layers.Input(shape=input_shape, dtype=tf.float64, name="input")
...
def create_mixed_data_network(dim_mlp,
    ...
    mlp = create_mlp(dim_mlp, time_horizon)
    ...
    x= tf.keras.layers.Concatenate()([mlp.output, cnn.output]) #combine outputs 
    #x = cnn.output
    ...
    #   x = tf.keras.layers.Dropout(0.2)(x)
    #if time_horizon<150:
    x = tf.keras.layers.Dense(256, activation='tanh', name='dense_b')(x)
    x = tf.keras.layers.Dropout(0.4)(x)
    #if time_horizon<80:
    #    x = tf.keras.layers.Dense(128,  activation='relu',name='dense_c')(x)
    #    x = tf.keras.layers.Dropout(0.2)(x)
    #if time_horizon<40:
    #    x = tf.keras.layers.Dense(64,  activation='relu',name='dense_d')(x)
    #    x = tf.keras.layers.Dropout(0.2)(x)
    #x = tf.keras.layers.Dropout(0.1)(x)
    if last_layer == 'softmax':
        x = tf.keras.layers.Dense(time_horizon, activation=activation,name='dense_z')(x)
        x = tf.keras.layers.Dropout(0.4)(x)
        x = tf.keras.layers.Softmax(name='output')(x)
    elif last_layer == 'sigmoid':
        x = tf.keras.layers.Dense(time_horizon, activation='sigmoid', name='output')(x)
    else: 
        x = tf.keras.layers.Dense(time_horizon, activation=last_layer,name='dense_z')(x)
    model=tf.keras.models.Model(inputs=[mlp.input, cnn.input], outputs=x,name='VNetSurvival_mixed_data')
    return model
