...
def change_labels_order(masks_filenames, path_output,
    ...
    """
    assert (len(old_labels_numbers) == len(old_labels_names) == len(
        new_labels_numbers)), "labels names, number or new labels are not same size"
...
def swap_mask_labels(mask, old_labels_numbers, new_labels_numbers):
    ...
    assert (np.all(
        condition_1 == condition_2)), "new label 0 (Background) is different than old label %s" % old_labels_numbers.index(
        new_labels_numbers[0])
