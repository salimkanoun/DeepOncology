...
def check_files(path):
    ...
    if not os.path.isdir(path):
        ...
        SHAPE_IMAGE = tf.expand_dims(
            tf.constant(sitk.GetArrayFromImage(sitk.ReadImage(filenames_scans[0], sitk.sitkFloat32)), dtype=tf.float32),
            axis=-1).shape
...
def check_ShapeDtype_MaskScan(filenames_masks, filenames_scans):
    ...
    for i, (filename_mask, filename_scan) in enumerate(zip(filenames_masks, filenames_scans)):
        ...
        assert (dtype_mask == mask.dtype), "Mask %s has dtype %s != %s" % (
        filename_mask, str(mask.dtype), str(dtype_mask))
        assert (dtype_scan == scan.dtype), "Scan %s has dtype %s != %s" % (
        filename_mask, str(scan.dtype), str(dtype_scan))
