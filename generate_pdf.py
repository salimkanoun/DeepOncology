...
dataset = dict()
dataset['train']['x'], dataset['train']['y'], dataset['val']['x'], dataset['val']['x'], dataset['test']['x'], dataset['test']['y'] = DM.get_train_val_test()

for subset_type in dataset.keys():
    # path to pdf to generate
    filename = os.join_path(MIP_folder, subset_type, 'MIP_preprocessed_{}_data.pdf'.format(subset_type))
