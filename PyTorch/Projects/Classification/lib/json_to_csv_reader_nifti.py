...
class JSON_TO_CSV: 
    ...
    def result_csv(self, nifti_directory, csv_directory): 
        """[summary]
        ...
        folder = os.listdir(nifti_directory) 
        for image in maj_data : 
            study_uid = image[0]
            for fold in folder : 
                folder2 = os.listdir(nifti_directory+fold)
                for fold2 in folder2:
                    if fold2[0]!='.':
                        folder3 = os.listdir(nifti_directory+fold+'/'+fold2)
                        for fold3 in folder3 : 
                            list_ct = os.listdir(nifti_directory+fold+'/'+fold2+'/'+fold3)
                            for i in list_ct:
                                if study_uid in i and '_CT' in i: 
                                    image.append(nifti_directory+fold+'/'+fold2+'/'+fold3+'/'+i)
